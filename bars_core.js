/* --- BARS AUTHORITATIVE MASTER CORE v29.0 --- */
...
window.showPage = function (pageId, el) {
    ...
    if (pageId === 'settings' && window.restoreSettingsUI) window.restoreSettingsUI();
...
async function initDashboard() {
    ...
    try {
        ...
        // Authoritative Session Sync: Update local session with fresh profile data from cloud
        if (data.profile) {
            BarsSession.save(token, { ...user, ...data.profile });
        }
        ...
        sidebar.innerHTML = items.map(item => {
            ...
            if (item.hideForMentors && (user.role === 'Mentor' || user.role === 'mentor')) return '';
        ...
        // Profile Sync (Robust)
        const activeUser = BarsSession.get()?.user || user;
        const profName = document.getElementById('profile-name');
        if (profName) profName.textContent = activeUser.full_name || activeUser.name || 'User';

        const profInitial = document.getElementById('profile-initial');
        if (profInitial) profInitial.textContent = (activeUser.full_name || activeUser.name || 'U').charAt(0).toUpperCase();
        ...
        if (fn) fn.textContent = activeUser.first_name || activeUser.firstName || (activeUser.full_name ? activeUser.full_name.split(' ')[0] : '—');
        if (ln) ln.textContent = activeUser.last_name || activeUser.lastName || (activeUser.full_name ? activeUser.full_name.split(' ')[1] : '—');
        if (em) em.textContent = activeUser.email || '—';
                        ...
                        <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                            <div style="display: flex; align-items: center; gap: 2.5rem;">
                                ...
                                <div style="text-align: left;">
                                    ...
                                    <h3 style="margin: 0; color: #D4AF37; font-size: 1.85rem; font-weight: 800; letter-spacing: -0.5px;">${p.name || "User"}</h3>
...
window.syncChat = async function () {
    ...
    const res = await fetch(`/api/messages?pair_id=${window.CURRENT_CHAT_PAIR}`, { headers: { 'Authorization': `Bearer ${BarsSession.get().token}` } });
...
window.initDiagnosticOverlay = () => {
    ...
    logAPI('BOOT', 'System Online', 200, 'v141.0 Restoration Master');
};

window.saveSettings = () => {
    logAPI('SETTINGS', 'Global Save', 200, 'Preferences Persisted');
    alert("✓ Settings saved successfully!");
};

window.saveSetting = (el) => {
    logAPI('SETTINGS', 'Setting Updated', 200, el.checked ? 'Enabled' : 'Disabled');
    const parent = el.closest('.setting-item-v2') || el.parentElement;
    if (parent) {
        parent.style.setProperty('border-color', 'var(--bars-gold)');
        setTimeout(() => parent.style.setProperty('border-color', '#f1f5f9'), 1000);
    }
};
...
window.toggleSetting = function(el, key) {
    ...
    localStorage.setItem(`bars_setting_${key}`, isActive ? 'true' : 'false');
    logAPI('SETTINGS', 'Toggle', 200, `${key}: ${isActive ? 'ON' : 'OFF'}`);
};

window.restoreSettingsUI = () => {
    ['master', 'resources', 'sessions', 'messages'].forEach(key => {
        const val = localStorage.getItem(`bars_setting_${key}`);
        const el = document.getElementById(`toggle-${key}`);
        if (el && val === 'true') el.classList.add('active');
...
window.submitResourceUpload = async function () {
    ...
    logAPI('UPLOAD', '/api/resources/upload-file', 0, 'PENDING');
    ...
    try {
        ...
        logAPI('UPLOAD', '/api/resources/upload-file', response.status, data.success ? 'Success' : (data.error || 'Server error'));

        if (data.success) {
            alert("✓ Resource uploaded successfully!");
            window.closeResourceUpload();
            if (window.DASH_DATA && window.DASH_DATA.resources) {
                // Fetch fresh data or just reload
        ...
        logAPI('ERR', '/api/resources/upload-file', 'Fail', e.message);
        alert("❌ Connectivity Error: " + e.message);
    } finally {
    ...
    try {
        ...
        container.innerHTML = notes.map(n => `
            <div class="resource-card" style="margin-bottom:1.5rem; background:white; padding:1.5rem; border-radius:20px; border:1.5px solid #f1f5f9;">
                ...
                <p style="font-size:1.1rem; color:#1e293b; line-height:1.6; font-weight:500; margin-bottom:1.5rem;">${n.note || n.content || ''}</p>
                <div style="border-top:1px solid #f8fafc; padding-top:1rem; display:flex; justify-content:space-between; align-items:center;">
                    <div style="display:flex; flex-direction:column;">
                        <span style="font-size:0.85rem; font-weight:800; color:#1e293b;">${n.mentor_name || n.created_by || 'Unknown Mentor'}</span>
                        <span style="font-size:0.75rem; color:#94a3b8; font-weight:500;">${n.mentor_email || n.email || ''}</span>
                    </div>
                    <div style="font-size:0.75rem; color:#D4AF37; font-weight:700;">Verified Entry</div>
                </div>
            </div>
        `).join('');
