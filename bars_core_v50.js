/* --- BARS AUTHORITATIVE MASTER CORE v95.0 --- */
...
window.initDashboard = function() { console.error("initDashboard called before core ready."); };
window.showAuthForm = function(id) { console.error("showAuthForm called before core ready."); };
window.dismissLoader = function() { console.error("dismissLoader called before core ready."); };
...
window.initDiagnosticOverlay = () => {
    ...
    const u = BarsSession.get()?.user;
    if (u && (u.role === 'ProgramStaff' || u.isCounselor)) {
    ...
    logAPI('BOOT', 'System Online', 200, 'v95.0 Foundation Ready');
};
...
window.submitResourceUpload = async () => {
    ...
    try {
        ...
        if (fileInput.files[0]) {
            ...
            // Ultra-strict sanitization: Alphanumeric only, no dots in body
            const bodyName = file.name.split('.')[0].replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
            const extension = file.name.split('.').pop().toLowerCase() || 'pdf';
            const fileName = `${Date.now()}_${bodyName}.${extension}`;
            
            log(`Authoritative Upload: ${fileName}`);
            const client = getSupabase();
            ...
            if (uploadError) {
                ...
                if (msg.includes("row-level security")) {
                    msg = "Permission Denied (RLS Policy). Please ensure your Supabase bucket 'resource-files' has an INSERT policy enabled for public uploads.";
                }
                throw new Error(msg);
            }
            ...
            log(`File available at: ${finalUrl}`);
        }
